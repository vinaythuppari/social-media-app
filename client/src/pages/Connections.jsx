...
import {User, UserPlus, UserCheck, UserRoundPen, MessageSquare, Users} from 'lucide-react'
        ...
        {dataArray.find((item)=>item.label === currentTab).value.map((user)=>(
          <div key={user._id}
           className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
            <img src={user.profile_picture} alt="user pic" className='rounded-full w-12 h-12 shadow-md mx-auto' />
              ...
              <div className='flex max-sm:flex-col gap-2 mt-4'>
                {
                  ...
                  className='w-full p-2 text-sm rounded bg-gradient-to-r from-indigo-500 to-purple-600
                   hover:from-indigo-600 hover:to-purple-700 active:scale-95 trasition text-white cursor-pointer'>
                    View Profile
                  </button>
                }
                {
                ...
                }
                {
                  currentTab === 'Connections' && (
                    ...
                    className='w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200 text-slate-800 active:scale-95
                    trasition cursor-pointer flex items-center justify-center gap-1'>
